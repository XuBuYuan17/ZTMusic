...
for (const file of ['./cache.js', './history.js', './settings.js']) {
  const source = await readFile(new URL(file, import.meta.url), 'utf8')
  assert.match(source, /import \{[^}]*ensureDB[^}]*\} from '\.\/init\.js'/s, `${file} must use the shared DB initializer`)
...
const messages = await readFile(new URL('../services/message-read-state.js', import.meta.url), 'utf8')
assert.doesNotMatch(messages, /setStorage\(/, 'message state must not write a second storage backend')

const historyBridge = await readFile(new URL('../player/history.js', import.meta.url), 'utf8')
