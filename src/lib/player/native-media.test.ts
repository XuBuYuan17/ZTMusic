...
const player = await readFile(new URL('src/lib/stores/player.svelte.ts', root), 'utf8')
...
assert.ok(player.includes('navigator.mediaSession.metadata = new MediaMetadata'), 'web builds must publish Media Session metadata')
assert.ok(player.includes("this._setMediaActionHandler('nexttrack'"), 'web builds must expose Media Session controls')
assert.match(
...
console.log('native media backend self-check: 12 assertions passed')
