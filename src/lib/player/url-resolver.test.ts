 ...
 * Run: node --experimental-strip-types src/lib/player/url-resolver.test.ts
 */

import { ncm } from '../api/client.ts'
import { dbCache } from '../db/cache.ts'
import { FALLBACK_URL_TEMPLATE } from '../utils/constants.ts'
import { fillFallbackUrls, getPlayableUrls, type QualityUpgradeEvent } from './url-resolver.ts'
...
await runTest(async () => {
  ...
  dbCache.urlGet = async () => null
  dbCache.urlSet = async (_id, urls) => { cachedUrls = urls }
  ncm.songUrl = async () => ({ data: [{ url: '' }] })
