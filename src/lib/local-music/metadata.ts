...
/** 文件名/类型/大小/修改时间的最小结构：测试传裸对象，真实调用传 File */
interface FileIdentityLike {
  name?: unknown
  type?: unknown
  size?: unknown
...
function asFileIdentity(file: unknown): FileIdentityLike {
  return file && typeof file === 'object' ? file as FileIdentityLike : {}
}
...
export function isSupportedAudioFile(file: unknown): boolean {
  const f = asFileIdentity(file)
  const mime = String(f.type || '').toLowerCase()
  return mime.startsWith('audio/') || AUDIO_EXTENSIONS.has(extensionOf(f.name))
}

export function createLocalTrackId(file: unknown): string {
  const f = asFileIdentity(file)
  const source = `${f.name || ''}\0${f.size || 0}\0${f.lastModified || 0}`
  let hash = 2166136261
...
export function parseId3Metadata(buffer: ArrayBuffer | Uint8Array | null | undefined): Partial<LocalMetadata> {
  const bytes = buffer instanceof Uint8Array ? buffer : buffer ? new Uint8Array(buffer) : new Uint8Array(0)
  ...
  const result: Partial<LocalMetadata> = {}
  const frameMap: Record<string, keyof LocalMetadata> = { TIT2: 'title', TPE1: 'artist', TALB: 'album' }
  let offset = 10
...
export function fallbackMetadata(fileName: unknown = ''): LocalMetadata {
...
export async function readLocalMetadata(file: File): Promise<LocalMetadata> {
