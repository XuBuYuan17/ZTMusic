...
import { MusicProviderRegistry, defineMusicProvider, type MusicProviderDefinition } from './provider.ts'

assert.throws(() => defineMusicProvider({ id: 'Bad ID', name: 'bad', search() {} }), /stable lowercase id/)
// 故意传缺 search 的非法对象，验证运行时校验
assert.throws(() => defineMusicProvider({ id: 'empty', name: 'Empty' } as MusicProviderDefinition), /must implement search/)
