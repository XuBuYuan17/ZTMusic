...
<div class="pc" style="gap:{gap}">
  <!-- 随机播放 -->
  <button
    class="pc-btn"
    class:active={mode === 'shuffle'}
    onclick={onshuffle}
    aria-label="随机播放"
    disabled={disabled && mode !== 'shuffle'}
  >
  ...
  <!-- 上一首 -->
  <button class="pc-btn" onclick={onprev} aria-label="上一首" disabled={disabled}>
  ...
  <!-- 播放/暂停 -->
  <button class="pc-btn pc-btn--play" onclick={onplaypause} aria-label={playing ? '暂停' : '播放'} disabled={disabled}>
    {#if loading}
      <Spinner size={size === 'lg' ? 'md' : 'sm'} />
    {:else if playing}
      <svg viewBox="0 0 24 24" width={size === 'lg' ? 28 : 24} height={size === 'lg' ? 28 : 24} fill="currentColor">
  ...
  <!-- 下一首 -->
  <button class="pc-btn" onclick={onnext} aria-label="下一首" disabled={disabled}>
    <svg viewBox="0 0 24 24" width={sz} height={sz} fill="currentColor">
  ...
  <button
    class="pc-btn"
    class:active={mode === 'repeat'}
    onclick={onrepeat}
    aria-label="单曲循环"
    disabled={disabled && mode !== 'repeat'}
  >
