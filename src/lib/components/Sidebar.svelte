...
<aside class="sidebar" class:collapsed>
  <div class="sidebar-header">
    <button class="sidebar-toggle" onclick={() => collapsed = !collapsed} title={collapsed ? '展开侧栏' : '收起侧栏'}>
    ...
    <span class="sidebar-logo">LyricEase</span>
  </div>

  <nav class="sidebar-nav">
    <div class="nav-group-label">发现</div>

    <button class="nav-item" class:active={activeView === 'home'} onclick={() => nav('home')}>
      <span class="nav-icon">
        {#if auth.isLoggedIn && auth.user?.avatarUrl}
          <img class="user-avatar-nav" src={auth.user.avatarUrl + `?param=40y40&_=${refreshKey}`} alt="" referrerpolicy="no-referrer" />
        {:else}
          <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><path d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
        {/if}
      </span>
    ...
    <button class="nav-item" class:active={activeView === 'fm'} onclick={() => nav('fm')}>
      ...
      <span class="nav-label">私人FM</span>
    </button>
    ...
    <button class="nav-item" class:active={activeView === 'library'} onclick={() => nav('library')}>
      <span class="nav-icon">
        <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
      </span>
    ...
    <button class="nav-item" class:active={activeView === 'cloud'} onclick={() => nav('cloud')}>
      <span class="nav-icon">
        <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><path d="M17.5 19H9a7 7 0 1 1 6.71-9h1.79a4.5 4.5 0 1 1 0 9z"/></svg>
      </span>
      <span class="nav-label">音乐云盘</span>
    </button>

    <button class="nav-item" class:active={activeView === 'recent'} onclick={() => nav('recent')}>
      <span class="nav-icon">
        <svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
      </span>
