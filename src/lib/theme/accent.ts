export type AccentThemeName = 'red' | 'berry' | 'violet' | 'blue' | 'teal' | 'orange' | 'cover'

interface AccentOption {
  value: AccentThemeName
  label: string
  preview: string
}
...
} satisfies Record<Exclude<AccentThemeName, 'cover'>, Rgb>

const extractionCache = new Map<string, Promise<Rgb | null>>()

export function normalizeAccentTheme(value: unknown): AccentThemeName {
  return ACCENT_THEME_OPTIONS.some(option => option.value === value) ? value as AccentThemeName : 'red'
}
...
export function adaptAccentColor(color: Rgb, theme: string = 'dark'): Rgb {
...
export function pickAccentColor(imageData: Uint8ClampedArray | Uint8Array): Rgb | null {
  const bins: AccentBin[] = Array.from({ length: 24 }, () => ({ weight: 0, r: 0, g: 0, b: 0 }))
  for (let i = 0; i < imageData.length; i += 16) {
    const color: Rgb = { r: imageData[i]!, g: imageData[i + 1]!, b: imageData[i + 2]! }
...
export function buildAccentProperties(color: Rgb, theme: string = 'dark'): Record<string, string> {
  ...
  const alpha = (value: string) => `rgb(${main.r} ${main.g} ${main.b} / ${value})`
  return {
    '--accent': css(main),
...
export function getAccentProperties(name: unknown, theme: string = 'dark', coverColor: Rgb | null = null): Record<string, string> {
  const normalized = normalizeAccentTheme(name)
  return buildAccentProperties(normalized === 'cover' && coverColor ? coverColor : PRESETS[normalized as keyof typeof PRESETS] || PRESETS.red, theme)
}

export function applyAccentProperties(element: HTMLElement, properties: Record<string, string>): void {
  for (const [name, value] of Object.entries(properties)) element.style.setProperty(name, value)
}

export async function extractCoverAccent(url: string | null | undefined): Promise<Rgb | null> {
