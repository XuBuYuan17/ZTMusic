/**
 * 运行时探测自检。
 * 跑法：node src/lib/utils/runtime.test.js
 * 退出码：0 = 过，1 = 挂。
 */
import assert from 'node:assert/strict'
import { isTauriAndroid, isTauriDesktop, isTauriRuntime, runtimePlatform } from './runtime.js'

// 备份原始 navigator 状态
const originalWindow = globalThis.window
const originalNavigatorDescriptor = Object.getOwnPropertyDescriptor(globalThis, 'navigator')

function setRuntime({ hasTauri = false, platform = '', userAgent = '' } = {}) {
  globalThis.window = hasTauri ? { __TAURI_INTERNALS__: {} } : { __TAURI_INTERNALS__: undefined }
