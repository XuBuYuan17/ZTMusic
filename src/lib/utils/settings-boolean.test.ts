/**
 * 布尔设置项的取值契约。
 * Run: node src/lib/utils/settings-boolean.test.ts
 *
...
const { getBooleanSetting, getSetting, setBooleanSetting, setSetting } = await import('./settings.ts')
...
check(!!falseString === true, "the string 'false' is truthy — must compare, not coerce")
...
const composable = await readFile(new URL('../composables/useSettings.svelte.ts', import.meta.url), 'utf8')
