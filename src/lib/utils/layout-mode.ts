...
export function getLayoutMode(): string {
  return getSetting(LAYOUT_MODE_KEY, 'auto')
}

export function setLayoutMode(value: string): string {
...
export interface LayoutModeStore {
  subscribe(fn: (state: LayoutState) => void): Unsubscriber
}
 ...
 */
function createLayoutModeStore(): LayoutModeStore {
  if (!isBrowser) {
    return {
      subscribe: (fn) => {
  ...
  // 启动时多次校验，确保窗口尺寸就绪
  const scheduleUpdate = (ms: number) => setTimeout(update, ms)
